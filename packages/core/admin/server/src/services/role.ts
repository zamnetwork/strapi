...
const create = async (attributes: Partial<AdminRole>): Promise<AdminRole> => {
  ...
  strapi.get('eventHub').emit('role.create', { role: sanitizeRole(result) });
...
const update = async (params: any, attributes: Partial<AdminRole>): Promise<AdminRole> => {
  ...
  strapi.get('eventHub').emit('role.update', { role: sanitizeRole(result) });
...
const deleteByIds = async (ids = [] as Entity.ID[]): Promise<AdminRole[]> => {
  ...
  for (const id of ids) {
    ...
    if (deletedRole) {
      strapi.get('eventHub').emit('role.delete', { role: deletedRole });
